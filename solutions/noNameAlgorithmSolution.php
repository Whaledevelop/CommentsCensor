<?php
   ...
   * Этот массив имеет индекс равный этой позиции.
   * 
  */
  function getLettersArraysOfPatterns(array $patterns) : array {
    ...
    foreach ($patterns as $pattern) {
      for ($i = 0; $i < $max_length; $i++) {
        $lettersArrays[$i][] = empty($pattern[$i]) ? "" : $pattern[$i];
      }    
    }
  ...
  /**
   * Изначально идея алгоритма возникла из желания как-то объединить поиск по 
   * нескольким паттернам. Я представлял его похожим на наивный алгоритм,
   * но в процессе реализации все изменилось. Алгоритму не хватает чего-то вроде
   * префикс функции из алгоритма КМП для сокращения кол-ва итераций при 
   * частичном совпадении строки и паттерна. Я планирую его еще усовершенствовать
   */
  ...
  function noNameAlgorithmSolution(string $string, array $patterns) {
    ...
    // Индекс массива первого порядка массива $lettersArrays
    $j = 0;
    // k - индекс символа в подстроке среди символов других паттернов
    // Индекс массива второго порядка массива $lettersArrays
    $k = 0;
